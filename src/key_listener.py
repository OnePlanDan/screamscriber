...
class PynputBackend(InputBackend):
    ...
    def _create_key_map(self):
        ...
        pynput exposes a different subset of Key attributes per OS. macOS
        in particular doesn't have Key.insert / num_lock / scroll_lock /
        pause / print_screen. Build the map programmatically and skip
        attributes that don't exist on the current platform.
        """
        Key = self.keyboard.Key
        KC = self.keyboard.KeyCode
        Btn = self.mouse.Button

        key_map = {}

        named_keys = {
            'ctrl_l': KeyCode.CTRL_LEFT, 'ctrl_r': KeyCode.CTRL_RIGHT,
            'shift_l': KeyCode.SHIFT_LEFT, 'shift_r': KeyCode.SHIFT_RIGHT,
            'alt_l': KeyCode.ALT_LEFT, 'alt_r': KeyCode.ALT_RIGHT,
            'cmd_l': KeyCode.META_LEFT, 'cmd_r': KeyCode.META_RIGHT,
            ...
            'backspace': KeyCode.BACKSPACE, 'esc': KeyCode.ESC,
            ...
            'num_lock': KeyCode.NUM_LOCK, 'scroll_lock': KeyCode.SCROLL_LOCK,
            'pause': KeyCode.PAUSE, 'print_screen': KeyCode.PRINT_SCREEN,
        ...
        digit_codes = [KeyCode.ZERO, KeyCode.ONE, KeyCode.TWO, KeyCode.THREE, KeyCode.FOUR,
                       KeyCode.FIVE, KeyCode.SIX, KeyCode.SEVEN, KeyCode.EIGHT, KeyCode.NINE]
        for d in range(10):
            key_map[KC.from_char(str(d))] = digit_codes[d]

        for ch in 'abcdefghijklmnopqrstuvwxyz':
            key_map[KC.from_char(ch)] = KeyCode[ch.upper()]
        ...
        numpad_vk = [
            ...
            (107, KeyCode.NUMPAD_ADD), (109, KeyCode.NUMPAD_SUBTRACT),
            (106, KeyCode.NUMPAD_MULTIPLY), (111, KeyCode.NUMPAD_DIVIDE),
            (110, KeyCode.NUMPAD_DECIMAL),
        ]
        for vk, code in numpad_vk:
            key_map[KC.from_vk(vk)] = code
        ...
        for ch, code in punctuation:
            key_map[KC.from_char(ch)] = code

        key_map[Btn.left] = KeyCode.MOUSE_LEFT
        key_map[Btn.right] = KeyCode.MOUSE_RIGHT
        key_map[Btn.middle] = KeyCode.MOUSE_MIDDLE
